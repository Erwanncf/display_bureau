...
String url = "https://liveobjects.orange-business.com/api/v0/data/streams/urn:lo:nsid:smartDisplay:1856745!Data?limit=1";
String ApiKey = "7ef7949fb37c454aa3087f63c276573d";
...
{
  {"Temp", "Hygro", "PsAt", "Lum", "BruitSalleCrea", "Alt", "tempCaf", "Message", "Heure", "Date", "IP","RSSI","Box","BruitBureau"},
 ...
 if (currentMillis - previousMillis >= interval)
  {
    previousMillis = currentMillis;
     ...
     // dht11.read(humidite, temperatur);
      ...
      sensor [1] [13] = String(maxPeakToPeak) ;
  }
}
...
void matrixDisplay ()
{
  ...
  while (lock != 1)
  {
    msgScrol =  msgScrol + 1;
    //msgScrol ==6 |
    if ( msgScrol == 3 | msgScrol == 4 | msgScrol == 5 | msgScrol == 10| msgScrol == 11| msgScrol == 12)
    {
      lock = 0;
    }
    else
...
void GetLiveObjectData()
{
  ...
  Serial.print("Live objecvt json: ");
...
void LiveObjectDataParser()
{
    ...
    char charTempCaf[1];
    int intTempCaf=0;
    memset(charTempCaf,'\0',1);
    
     for(int i=0;i<Jsonlength;i++)
     { 
       if(jsonArayOK[i]=='t'&& jsonArayOK[i+1]=='e'&& jsonArayOK[i+2]=='m'&& jsonArayOK[i+3]=='p'&& jsonArayOK[i+4]=='C'&& jsonArayOK[i+5]=='a'&& jsonArayOK[i+6]=='f'&& jsonArayOK[i+7]=='"'&& jsonArayOK[i+8]==':')
       {
          Serial.println("Temp caf match");
          charTempCaf[0]=jsonArayOK[i+9];
          
          if (isDigit(jsonArayOK[i+10]))
          {
            charTempCaf[1]=jsonArayOK[i+10];
          }
          else 
          {
            charTempCaf[1]='\0';
    ...
    intTempCaf = atoi(charTempCaf);
    sensor [1] [6] = String(intTempCaf) ;
    Serial.print("intTempCaf: ");
    Serial.println(intTempCaf);
    
    char charBruit[2];
    int intBruit =0;
    memset(charBruit,'\0',2);
    
     
      for(int i=0;i<Jsonlength;i++)
     { 
       if(jsonArayOK[i]=='B'&& jsonArayOK[i+1]=='r'&& jsonArayOK[i+2]=='u'&& jsonArayOK[i+3]=='i'&& jsonArayOK[i+4]=='t'&& jsonArayOK[i+5]=='"'&& jsonArayOK[i+6]==':')
       {
          Serial.println("bruit match");
          charBruit[0]=jsonArayOK[i+7];
            
          if (isDigit(jsonArayOK[i+8]))
          {
            charBruit[1]=jsonArayOK[i+8];
          }
          else 
          {
            charBruit[1]='\0';
    ...
    intBruit = atoi(charBruit);
    sensor [1] [4] = String(intBruit) ;
    Serial.print("intBruit: ");
    Serial.println(intBruit);
}
